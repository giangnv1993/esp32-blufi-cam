...
#define CONFIG_SPIFFS_BASE_ADDR				180000
...
int spiffs_mount() {
    ...
    printf("Mounting SPIFFS files system");
    ...
    printf("Start address: 0x%x; Size %d KB", cfg.phys_addr, cfg.phys_size / 1024);
    printf("  Work buffer: %d B", cfg.log_page_size * 8);
    printf("   FDS buffer: %d B", sizeof(spiffs_fd) * SPIFFS_TEMPORAL_CACHE_HIT_SCORE);
    printf("   Cache size: %d B", cfg.log_page_size * SPIFFS_TEMPORAL_CACHE_HIT_SCORE);
    while (retries < 2) {
		res = SPIFFS_mount(
		...
		if (res < 0) {
			if (fs.err_code == SPIFFS_ERR_NOT_A_FS) {
		        ...
		        free(my_spiffs_fds);
		        free(my_spiffs_cache);
				printf("Error mounting fs (%d)", res);
				goto exit;
			}
		}
		else break;
...
err_exit:
	printf("Error allocating fs structures (%d)", err);
exit:
...
void vfs_spiffs_register() {
    ...
    printf("Registering SPIFFS file system");
